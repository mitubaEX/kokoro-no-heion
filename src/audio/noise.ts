...
/**
 * 指定タイプ・指定長のノイズサンプル列を生成する純関数。
 * テスト容易性のため AudioContext には依存しない。
 */
export function generateNoiseSamples(type: NoiseType, length: number): Float32Array {
  const data = new Float32Array(Math.max(0, length))
  ...
  if (type === 'brown') {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // 3.5 倍すると稀に >1 に振れて IR/バッファを汚すので 0.99 にクランプ
      let v = last * 3.5
      if (v > 1) v = 1
      else if (v < -1) v = -1
      data[i] = v
...
export function createNoiseBuffer(
  ...
  const samples = generateNoiseSamples(type, length)
  data.set(samples)
  return buffer
}
